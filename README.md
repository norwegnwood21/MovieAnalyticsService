...
| Параметр | Значение |
|----------|----------|
| **Предусловия** | Приложение запущено, база данных PostgreSQL подключена, страница `http://localhost:8080` открыта |
| **Шаги** | 1. В поле «Логин» ввести `testuser`<br>2. В поле «Email» ввести `test@example.com`<br>3. В поле «Пароль» ввести `123456`<br>4. Нажать кнопку «Зарегистрироваться»<br>5. В форме входа ввести логин `testuser` и пароль `123456`<br>6. Нажать «Войти» |
| **Ожидаемый результат** | После регистрации появляется сообщение об успехе. После входа интерфейс переключается в режим «Авторизован», отображается имя пользователя, становятся доступны поля ввода фильмов и кнопка «Запустить анализ». |

### ТК-2: Создание аналитического отчёта по списку фильмов

| Параметр | Значение |
|----------|----------|
| **Предусловия** | Пользователь авторизован, подключение к TMDB API работает, токен валиден |
| **Шаги** | 1. В текстовое поле «Введите названия фильмов (каждый с новой строки)» ввести:<br>   `Матрица`<br>   `Начало`<br>2. Нажать кнопку «Запустить анализ»<br>3. Дождаться окончания обработки (статус сменится с RUNNING на COMPLETED)<br>4. Просмотреть результат |
| **Ожидаемый результат** | Появляется блок с результатами: краткая информация по каждому фильму (название, год, жанры, актёры), а также три диаграммы (распределение по жанрам, по годам выпуска, по актёрам). Статус отчёта – «COMPLETED». |

### ТК-3: Повторный анализ ранее сохранённого отчёта без обращения к TMDB

| Параметр | Значение |
|----------|----------|
| **Предусловия** | У пользователя есть хотя бы один ранее созданный отчёт (например, из ТК-2). Отчёт имеет статус COMPLETED. |
| **Шаги** | 1. В разделе «История отчётов» найти нужный отчёт<br>2. Нажать кнопку «Повторить без TMDB»<br>3. Дождаться создания нового отчёта<br>4. Сравнить новый отчёт с исходным |
| **Ожидаемый результат** | В истории появляется новый отчёт с тем же списком фильмов и такой же аналитикой (жанры, годы, актёры). При этом в логах `external_api_logs` не появляется новых записей (повторные HTTP-запросы к TMDB не выполняются). Статус нового отчёта – «COMPLETED». |

## 16. Скриншоты
Ниже приведены скриншоты работы программы
**Рис. 1. Форма регистрации и входа**  
![Форма регистрации и входа](Screenshots/screen1.png)

**Рис. 2. Ввод списка фильмов**  
![Ввод списка фильмов](Screenshots/screen2.png)

**Рис. 3. Результат анализа с диаграммами**  
![Результат анализа](Screenshots/screen3.png)

**Рис. 4. Swagger UI (документация API)**  
![Swagger UI](Screenshots/screen4.png)

**Рис. 5. Выполнение тестов (mvn test)**  
![Выполнение тестов](Screenshots/screen5.png)
